...
import { buildAnkiGlossaryHtml } from '@/Manatan/utils/customCss';
import {
...
const AnkiButtons: React.FC<{
    ...
    const prepareNoteData = async (croppedBase64?: string) => {
        ...
        const generateElementHTML = (
            ...
            const dataAttrs = data && typeof data === 'object'
                ? Object.entries(data).map(([key, value]) => {
                    if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') {
                        return `data-sc-${key}="${escapeHtmlAttr(String(value))}"`;
                    }
                    return '';
                }).filter(Boolean).join(' ')
                : '';
            ...
            const styleAttr = finalStyle ? ` style="${escapeHtmlAttr(finalStyle)}"` : '';
            ...
            if (config.void || VOID_TAGS_HTML.includes(tag)) {
                if (tag === 'img') {
                    ...
                    const src = rawPath && glossaryDictionaryName
                        ...
                        : rawPath;
                    const alt = node.alt || '';
                    return `<img src="${escapeHtmlAttr(src)}" alt="${escapeHtmlAttr(alt)}"${styleAttr} ${dataAttrs}${classAttr} />`;
                }
                if (tag === 'br') return '<br />';
            ...
            if (tag === 'a') {
                return `<a href="${escapeHtmlAttr(href || '')}" target="_blank" style="${escapeHtmlAttr(`text-decoration: underline;${dictStyle}`)}" ${dataAttrs}${classAttr}>${generateHTML(content, glossaryDictionaryName)}</a>`;
            }
        ...
        const buildGlossaryHtml = (dictionaryName?: string): string => {
            ...
            const definitions = glossaryEntries.map((def) => {
